...
\begin{tikzpicture}[
    font=\sffamily\small,
    % Définition des styles de boites larges et détaillées
    box/.style={rectangle, draw=black!90, fill=white, very thick, rounded corners=8pt, align=center, minimum height=2.5cm, text width=6.5cm},
    frontend_style/.style={box, fill=cyan!5, draw=cyan!80!black},
    backend_style/.style={box, fill=orange!5, draw=orange!80!black},
    petri_style/.style={box, fill=red!5, draw=red!80!black},
    db_style/.style={cylinder, draw=black!80, fill=yellow!10, shape border rotate=90, aspect=0.15, minimum height=2cm, minimum width=3cm, align=center},
    % Styles des flèches avec épaisseur renforcée
    arrow/.style={->, >=latex, line width=1mm, draw=gray!70},
    ws_arrow/.style={<->, >=latex, line width=1mm, dashed, draw=purple!80},
    % Label des flèches
    label_style/.style={midway, fill=white, font=\scriptsize\bfseries, draw=gray!30, rounded corners=3pt, inner sep=4pt, align=center}
]

% ================= ESPACEMENT MASSIF (X de -10 à 10, Y de 0 à 20) =================

% --- NIVEAU 1 : PRÉSENTATION (Y = 20) ---
\node[frontend_style] (frontend) at (-7, 20) {
    \textbf{\Large FRONTEND (Client/Driver App)}\\
    \vspace{0.2cm}
    \begin{minipage}{6cm}
    \scriptsize
    \textbf{Composants UI}: Leaflet Map, NotificationBell, AuthForm\\
    \textbf{Logic}: React Query (Caching), Zustand (State)\\
    \textbf{Real-time}: SockJS Client (Topic Subscription)
    \end{minipage}
};

\node[frontend_style, draw=purple!80, fill=purple!2] (ws_client) at (7, 20) {
    \textbf{\Large GESTIONNAIRE WEBSOCKET}\\
    \vspace{0.2cm}
    \begin{minipage}{6cm}
    \scriptsize
    \textbf{Events}: \texttt{ON_CONNECT}, \texttt{ON_MESSAGE}\\
    \textbf{Topics}: \texttt{/topic/notifications/\{trackingCode\}}\\
    \textbf{Logic}: Heartbeat, Auto-reconnect, Payload Parsing
    \end{minipage}
};

% --- NIVEAU 2 : INTERFACE API (Y = 14) ---
\node[backend_style, text width=8cm, minimum height=2cm] (controllers) at (0, 14) {
    \textbf{\Large API LAYER (Controllers réactifs)}\\
    \vspace{0.1cm}
    \begin{minipage}{7.5cm}
    \scriptsize
    \textbf{Technologies}: Spring WebFlux, Project Reactor (\texttt{Mono}/\texttt{Flux})\\
    \textbf{Validation}: Bean Validation, Custom Constraints (Phone, Coordinates)
    \end{minipage}
};

% --- NIVEAU 3 : LOGIQUE MÉTIER & WORKFLOW (Y = 7) ---
\node[backend_style, text width=7.5cm, minimum height=4cm] (services) at (-6, 7) {
    \textbf{\Large DELIVERY SERVICES (Core Logic)}\\
    \vspace{0.2cm}
    \begin{minipage}{7cm}
    \scriptsize
    - \textbf{Routing}: Algorithme A* (Composite Cost Function)\\
    - \textbf{Optimization}: Google OR-Tools (Solveur CVRP)\\
    - \textbf{Tracking}: Filtre de Kalman (Prediction d'ETA)\\
    - \textbf{Hubs}: HubDepositService (Logic Dépôt/Retrait)\\
    - \textbf{Notifications}: NotificationService (Trigger Logic)
    \end{minipage}
};

\node[petri_style, text width=7.5cm, minimum height=4cm] (petri_net) at (8, 7) {
    \textbf{\Large PETRI NET WORKFLOW API}\\
    \vspace{0.2cm}
    \begin{minipage}{7cm}
    \scriptsize
    - \textbf{Engine}: Tir de transitions mathématiques\\
    - \textbf{Model}: Places (Dépôt, Transit, Hub) \& Transitions\\
    - \textbf{Safety}: Vérification formelle du cycle de vie\\
    - \textbf{Logic}: Empêcher les états impossibles (ex: Delivered avant PickedUp)
    \end{minipage}
};

% --- NIVEAU 4 : PERSISTANCE (Y = 0) ---
\node[db_style] (db_delivery) at (-6, 0) {
    \textbf{\Large DB DELIVERY}\\
    PostgreSQL + PostGIS\\
    \footnotesize Noeuds, Arcs routiers,\\
    Livraisons, Historique GPS
};

\node[db_style] (db_petri) at (8, 0) {
    \textbf{\Large DB PETRI NET}\\
    PostgreSQL\\
    \footnotesize Graphe du réseau de Petri,\\
    Tokens actifs, Marquages
};

% ================= FLÈCHES LONGUES AVEC TYPES DE DONNÉES PRÉCIS =================

% 1. Frontend -> Controllers (REST EXTÉRIEUR)
\draw[arrow] (frontend.south) .. controls (-7, 17) and (0, 17) .. (controllers.north)
    node[pos=0.5, label_style] {
    \textbf{HTTP POST/GET (JSON)}\\
    \texttt{DeliveryDTO}, \texttt{HubDepositRequest}\\
    \texttt{Coordinates \{lat, lng\}}
    };

% 2. Backend -> WS Broadcaster (INTERNE)
\draw[ws_arrow] (services.north) .. controls (-6, 17) and (7, 17) .. (ws_client.south)
    node[pos=0.6, label_style] {
    \textbf{INTERNAL PUSH (STOMP)}\\
    \texttt{NotificationDTO}, \texttt{TrackingUpdate}\\
    \texttt{Status: AT_HUB|DELIVERED}
    };

% 3. Gateway -> Services
\draw[arrow] (controllers.south) -- (services.north)
    node[pos=0.5, label_style] {Reactive Call (\texttt{flatMap})};

% 4. Services -> Petri Net API
\draw[arrow] (services.east) -- (petri_net.west)
    node[pos=0.5, label_style] {
    \textbf{HTTP POST}\\
    \texttt{TransitionRequest}\\
    \texttt{entityId}, \texttt{timestamp}
    };

% 5. Services -> Database
\draw[arrow] (services.south) -- (db_delivery.north)
    node[pos=0.5, label_style] {\textbf{R2DBC}: SQL Queries};

% 6. Petri Net -> Database
\draw[arrow] (petri_net.south) -- (db_petri.north)
    node[pos=0.5, label_style] {\textbf{JPA}: Entity Persistence};

% 7. Frontend -> WS Connection
\draw[ws_arrow, dashed] (frontend.east) -- (ws_client.west)
    node[pos=0.5, label_style] {Full Duplex Stream\\JSON Payloads};

% ================= ANNOTATIONS LATÉRALES =================
\node[anchor=west, font=\Large\bfseries, color=gray!50] at (13, 20) {PRESENTATION};
\node[anchor=west, font=\Large\bfseries, color=gray!50] at (13, 14) {API GATEWAY};
\node[anchor=west, font=\Large\bfseries, color=gray!50] at (13, 7) {CORE LOGIC};
\node[anchor=west, font=\Large\bfseries, color=gray!50] at (13, 0) {STORAGE};

\end{tikzpicture}
\caption{Architecture logicielle exhaustive du système TiibnTick : Interactions inter-services et typage des flux de données.}
