...
public class HubDepositService {
    ...
    public Mono<com.delivery.optimization.dto.HubPickupResponse> pickupPackageFromHub(
        ...
        return deliveryRepository.findByTrackingCode(request.getTrackingCode())
                .switchIfEmpty(Mono
                        .error(new IllegalArgumentException("Tracking code not found: " + request.getTrackingCode())))
                .flatMap(delivery -> {
                    // Vérifier que l'identité correspond (téléphone du destinataire)
                    if (!delivery.getRecipientPhone().equals(request.getPickupPhone())) {
                        log.warn("Phone number mismatch for tracking code: {}", request.getTrackingCode());
                        return Mono.error(new IllegalArgumentException(
                            ...
                            .flatMap(deposit -> {
                                ...
                                return hubDepositRepository.save(deposit)
                                        .flatMap(updatedDeposit -> {
                                            // Mettre à jour le statut de la livraison à IN_TRANSIT
                                            delivery.setStatus(Delivery.DeliveryStatus.IN_TRANSIT);
                                            delivery.setNewEntity(false);

                                            return deliveryRepository.save(delivery)
                                                    .flatMap(updatedDelivery -> nodeRepository
                                                            .findById(deposit.getHubNodeId())
                                                            .flatMap(hub -> {
                                                                // Notifier via WebSocket
                                                                webSocketBroadcaster.broadcastDeliveryUpdate(
                                                                        delivery.getId(),
                                                                        "PICKED_UP_FROM_HUB",
                                                                        "Colis récupéré au hub: " + hub.getName());
                                                                ...
                                                                return Mono.just(
                                                                        com.delivery.optimization.dto.HubPickupResponse
                                                                                .builder()
                                                                                .success(true)
                                                                                .depositId(updatedDeposit.getId())
                                                                                .deliveryId(delivery.getId())
                                                                                .trackingCode(
                                                                                ...
                                                                                .pickedUpBy(request.getPickupName())
                                                            ...
                                                            .defaultIfEmpty(
                                                                    com.delivery.optimization.dto.HubPickupResponse
                                                                            ...
                                                                            .trackingCode(delivery.getTrackingCode())
                                                                            .hubName("Hub")
                                                                            .pickupTime(updatedDeposit.getPickupTime())
                                                                            .pickedUpBy(request.getPickupName())
                                                                            .message("Colis récupéré avec succès")
                                                                            .build()));
                                        });
                ...
                .doOnSuccess(response -> log.info("Hub pickup successful for tracking: {}", request.getTrackingCode()))
                ...
                .flatMap(delivery -> hubDepositRepository.findByDeliveryId(delivery.getId())
                        .flatMap(deposit -> nodeRepository.findById(deposit.getHubNodeId())
                                .map(hub -> {
                                    ...
                                    String maskedPhone = maskPhoneNumber(delivery.getRecipientPhone());
                                    String maskedName = maskName(delivery.getRecipientName());

                                    return com.delivery.optimization.dto.HubParcelInfoDTO.builder()
                                            .trackingCode(delivery.getTrackingCode())
                                            .hubName(hub.getName())
                                            .recipientNameMasked(maskedName)
                                            .recipientPhoneMasked(maskedPhone)
                                            .status(deposit.getStatus())
                                            .storageLocation(deposit.getStorageLocation())
                                            .availableForPickup(deposit
                                                    .getStatus() == HubDeposit.DepositStatus.DEPOSITED ||
                                                    deposit.getStatus() == HubDeposit.DepositStatus.AWAITING_PICKUP)
                                            .depositTime(deposit.getDepositTime().toString())
                                            .packageDescription(delivery.getPackageDescription())
                                            .build();
                                }))
                        .switchIfEmpty(Mono.error(new IllegalArgumentException("Package not at hub"))));
    }
